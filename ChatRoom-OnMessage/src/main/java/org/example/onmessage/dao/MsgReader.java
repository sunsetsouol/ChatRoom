...
public class MsgReader {
    ...
    public MessageBO getMessageByClientId(WsMessageDTO wsMessageDTO) {
        List<MessageBO> lastZSetScore = redisCacheService.getLastZSetScore(RedisConstant.INBOX + wsMessageDTO.getFromUserId(), 50, MessageBO.class);
...
//        return getMsgByScore(key, globalMessageId, MessageBO.class);
    ...
    public Long getMaxClientId(Long fromUserId, Integer device) {
        List<WsMessageDTO> temMessage = redisCacheService.getLastZSetScore(RedisConstant.TEM_MESSAGE + fromUserId + ":" + device, 50L, WsMessageDTO.class);
        // 将反转后的消息再反转
